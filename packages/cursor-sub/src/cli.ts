...
async function requireCursorAgent(): Promise<void> {
  if (!(await cursorAgentExists())) {
    printErrorJson("cursor-agent not found", {
      install: "Install Cursor CLI: https://cursor.com/docs/cli",
    });
  }
}
...
program
  ...
  .description("Thin CLI wrapper around cursor-agent with a documented file substrate")
  .option("--dir <path>", "sessions root override (also CURSOR_SUB_HOME env)")
