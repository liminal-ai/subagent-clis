...
const slowStub = join(__dirname, "fixtures", "stub-cursor-agent-slow.mjs");

describe("passthrough", () => {
  it("passes bare --mode plan and suppresses --force/--sandbox defaults", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const { code } = await runCli(
        ["--dir", sessionsRoot, "exec", "plan this", "--mode", "plan"],
      ...
      expect(meta.argv).toContain("--trust");
      expect(meta.argv).toContain("--model");
    });
  });
  ...
  it("appends stream.jsonl incrementally before process exit", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const start = await runCli(
        ...
        { CURSOR_SUB_HOME: sessionsRoot, CURSOR_AGENT_BIN: slowStub },
      );
