...
describe("cli ux", () => {
  it("passes --version through to claude on exec", async () => {
  ...
  it("honors --dir after subcommand prompt", async () => {
    const sessionsRoot = await mkdtemp(join(tmpdir(), "claude-sub-dir-"));
    try {
      const { stdout, code } = await runCli(
  ...
  it("does not treat --dir after -- as wrapper session root", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const backendDir = await mkdtemp(join(tmpdir(), "claude-sub-backend-dir-"));
      try {
        const { stdout, code } = await runCli(
          [
            "--dir",
  ...
  it("result --timeout implies --wait and exits 4 on timeout", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const env = { CLAUDE_SUB_HOME: sessionsRoot, CLAUDE_BIN: sleepStub };
