...
describe("list", () => {
  it("lists recent runs as JSONL with session_id", async () => {
    await withTempSessions(async (sessionsRoot) => {
      ...
      expect(runs[0]!.session_id).toBe("claude-session-abc123");
  ...
  it("scopes list and latest resolution to cwd by default", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const cwdA = await mkdtemp(join(tmpdir(), "claude-sub-cwd-a-"));
      const cwdB = await mkdtemp(join(tmpdir(), "claude-sub-cwd-b-"));
      const cwdC = await mkdtemp(join(tmpdir(), "claude-sub-cwd-c-"));
      try {
        await runCli(
  ...
  it("stops a running detached run and writes error envelope", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const start = await runCli(
        ...
        { CLAUDE_SUB_HOME: sessionsRoot, CLAUDE_BIN: sleepStub },
      );
