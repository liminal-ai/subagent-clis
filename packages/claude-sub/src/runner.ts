...
import { buildClaudeExecArgs, buildClaudeArgv, extractModelFromArgs } from "./args.js";
import {
  createStreamState,
  mapRawEvent,
  sanitizeJsonLine,
...
function resolveClaudeSpawn(
  ...
  const bin = process.env.CLAUDE_BIN ?? "claude";
  const claudeArgs = [...execArgs, prompt];
  if (bin.endsWith(".mjs") || bin.endsWith(".js")) {
    return { command: process.execPath, argv: [bin, ...claudeArgs] };
  }
  return { command: bin, argv: claudeArgs };
}
...
export async function runSession(params: {
  ...
  const argv = buildClaudeArgv(prompt, passthrough);
