...
async function requireClaude(): Promise<void> {
  if (!(await claudeExists())) {
    printErrorJson("claude not found", {
...
program
  ...
  .description("Thin CLI wrapper around claude with a documented file substrate")
  .option("--dir <path>", "sessions root override (also CLAUDE_SUB_HOME env)")
