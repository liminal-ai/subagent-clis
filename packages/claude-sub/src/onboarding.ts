...
const CLI = "claude-subagent";

export function printOnboarding(): void {
  const text = `${CLI} ${VERSION} - Run Claude Code headlessly, return JSON envelopes

WHAT THIS DOES
  Thin wrapper over the local claude CLI (Claude Code). The calling agent decides
  what to build; ${CLI} executes the run, captures the full event stream,
...
SESSION DIRECTORY
  ...
  Override sessions root: --dir flag or CLAUDE_SUB_HOME env var (session file
  storage only — this does not choose where the run executes).
  Files per run:
    meta.json       Prompt, full argv, cwd, model, started_at. Written at spawn.
    raw.jsonl       Verbatim claude stream-json events, appended live.
    stream.jsonl    Canonical mapped events, each with ts, appended live.
    stderr.log      claude stderr; runner bootstrap errors land here too.
...
ENVELOPE FIELDS
  schema_version, backend ("claude"), run_id, session_id, model, cwd,
  status (ok|error), exit_code, result (final text from result event or last message),
  usage (from result event, includes total_cost_usd when reported, or null),
  duration_ms, started_at, ended_at, stream_path, raw_path, stderr_path,
...
EXECUTION PROFILE
  Always appends -p --output-format stream-json --verbose (claude requires
  --verbose with stream-json in -p mode; callers never hit that coupling error).
  Autonomy default: --dangerously-skip-permissions, suppressed when the caller
  supplies any permission posture (--permission-mode, --allowedTools/--allowed-tools,
  or --dangerously-skip-permissions itself). Use --permission-mode plan for
  read-only-ish analysis. Permission-mode choices: acceptEdits, auto,
  bypassPermissions, default, dontAsk, plan.
  No model default — the user's configured default applies; --model passes through.
  Runs load the target project's CLAUDE.md, settings, and hooks — behavior varies
  by repo.
  Flags after the prompt that aren't ${CLI}'s own (--dir, --text, --wait,
  --timeout, --prompt-file) pass through to claude verbatim; no -- needed (though supported).
  Examples: --permission-mode plan, --model haiku, --add-dir <path>,
  --session-id <uuid>, --continue, --resume <session-id>.
  --resume <session-id> continues a specific prior session; --continue resumes the most
  recent claude session in this directory (claude-native behavior, separate from --resume).

EXIT CODES
  0 ok | 1 CLI/usage error (includes unknown run_id) | 2 run failed | 3 result not ready | 4 wait timeout

SAFETY
  - Default profile is --dangerously-skip-permissions full autonomy — runs mutate
    the cwd with broad tool access. Expect a dirty git worktree afterward. Pass
    your own --permission-mode (e.g. plan) to suppress the default.
  - Every run calls a paid external CLI. Envelope usage includes total_cost_usd
  ...
  - Envelope status "ok" means claude exited cleanly and is_error was false — NOT
    that the work is correct. Read the diff and run tests before treating a task done.
