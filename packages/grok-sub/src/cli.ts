...
import { grokExists, runSession, spawnDetachedRunner } from "./runner.js";
import { validateResumePassthrough } from "./args.js";
import {
  ensureSessionDir,
  getPid,
  isSessionRunning,
...
async function requireGrok(): Promise<void> {
  if (!(await grokExists())) {
    printErrorJson("grok not found", {
      install: "Install Grok CLI and ensure `grok` is on PATH",
    });
  }
}
...
program
  .name("grok-subagent")
  .description("Thin CLI wrapper around grok with a documented file substrate")
  .option("--dir <path>", "sessions root override (also GROK_SUB_HOME env)")
...
program
  ...
  .option("--text", "print only the accumulated assistant text")
  .action(async function (this: Command) {
    ...
    const resumeError = validateResumePassthrough(passthrough);
    if (resumeError) {
      printErrorJson(resumeError);
    }
...
program
  .command("last")
  .description("Print accumulated assistant text as plain text")
  .argument("[run_id]", "run id (default: latest in this directory)")
  .option("--all", "resolve latest run across all directories")
  .action(async function (this: Command, runId?: string) {
    const opts = this.parent?.opts<GlobalOpts>() ?? {};
...
program
  .command("messages")
  .description("Print assistant text chunks separated by ---")
  .argument("[run_id]", "run id (default: latest in this directory)")
  .option("--all", "resolve latest run across all directories")
  .action(async function (this: Command, runId?: string) {
    const opts = this.parent?.opts<GlobalOpts>() ?? {};
