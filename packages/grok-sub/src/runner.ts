...
import { buildGrokExecArgs, buildGrokArgv, extractModelFromArgs } from "./args.js";
import {
  createStreamState,
  mapRawEvent,
  sanitizeJsonLine,
...
export async function grokExists(): Promise<boolean> {
...
export async function runSession(params: {
  ...
  const argv = buildGrokArgv(prompt, passthrough);
  ...
  try {
    ...
    const { command, argv: grokArgv } = resolveGrokSpawn(execArgs);
    stderrStream = createWriteStream(paths.stderr, { flags: "a" });

    await new Promise<void>((resolve, reject) => {
      const child = spawn(command, grokArgv, {
        cwd,
