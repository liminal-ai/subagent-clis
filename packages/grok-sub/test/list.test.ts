...
describe("list", () => {
  it("lists recent runs as JSONL with session_id", async () => {
    await withTempSessions(async (sessionsRoot) => {
      ...
      expect(runs[0]!.session_id).toBe("grok-session-abc123");
  ...
  it("scopes list and latest resolution to cwd by default", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const cwdA = await mkdtemp(join(tmpdir(), "grok-sub-cwd-a-"));
      const cwdB = await mkdtemp(join(tmpdir(), "grok-sub-cwd-b-"));
      const cwdC = await mkdtemp(join(tmpdir(), "grok-sub-cwd-c-"));
      try {
        await runCli(
  ...
  it("stops a running detached run and writes error envelope", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const start = await runCli(
        ...
        { GROK_SUB_HOME: sessionsRoot, GROK_BIN: sleepStub },
      );
