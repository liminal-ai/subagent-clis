...
describe("onboarding", () => {
  it("bare invocation exits 0 and prints section headers", async () => {
    ...
    expect(stdout).toContain("grok-subagent");
    expect(stdout).toContain("--always-approve");
    expect(stdout).toContain("--permission-mode");
    expect(stdout).toContain("streaming-json");
    expect(stdout).toContain("envelope.usage is usually null");
...
describe("docs", () => {
  ...
  it("prints non-empty events topic", async () => {
    ...
    expect(stdout).toContain("text {data:string}");
