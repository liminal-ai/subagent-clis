...
import { buildCodexExecArgs, buildCodexArgv, extractModelFromArgs } from "./args.js";
import {
  createStreamState,
  mapRawEvent,
  sanitizeJsonLine,
...
function resolveCodexSpawn(
  ...
  const bin = process.env.CODEX_BIN ?? "codex";
  const codexArgs = [...execArgs, prompt];
  if (bin.endsWith(".mjs") || bin.endsWith(".js")) {
    return { command: process.execPath, argv: [bin, ...codexArgs] };
  }
  return { command: bin, argv: codexArgs };
}
...
export async function runSession(params: {
  ...
  const argv = buildCodexArgv(prompt, passthrough);
  ...
  try {
    ...
    const { command, argv: codexArgv } = resolveCodexSpawn(execArgs, prompt);
    stderrStream = createWriteStream(paths.stderr, { flags: "a" });

    await new Promise<void>((resolve, reject) => {
      const child = spawn(command, codexArgv, {
        cwd,
