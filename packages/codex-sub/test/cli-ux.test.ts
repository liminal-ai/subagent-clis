...
describe("cli ux", () => {
  it("passes --version through to codex on exec", async () => {
  ...
  it("honors --dir after subcommand prompt", async () => {
    const sessionsRoot = await mkdtemp(join(tmpdir(), "codex-sub-dir-"));
    try {
      const { stdout, code } = await runCli(
  ...
  it("does not treat --dir after -- as wrapper session root", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const backendDir = await mkdtemp(join(tmpdir(), "codex-sub-backend-dir-"));
      try {
        const { stdout, code } = await runCli(
          [
            "--dir",
  ...
  it("result --timeout implies --wait and exits 4 on timeout", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const env = { CODEX_SUB_HOME: sessionsRoot, CODEX_BIN: sleepStub };
