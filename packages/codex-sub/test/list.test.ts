...
describe("list", () => {
  it("lists recent runs as JSONL with session_id", async () => {
    await withTempSessions(async (sessionsRoot) => {
      ...
      expect(runs[0]!.session_id).toBe("codex-thread-abc123");
  ...
  it("scopes list and latest resolution to cwd by default", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const cwdA = await mkdtemp(join(tmpdir(), "codex-sub-cwd-a-"));
      const cwdB = await mkdtemp(join(tmpdir(), "codex-sub-cwd-b-"));
      const cwdC = await mkdtemp(join(tmpdir(), "codex-sub-cwd-c-"));
      try {
        await runCli(
  ...
  it("stops a running detached run and writes error envelope", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const start = await runCli(
        ...
        { CODEX_SUB_HOME: sessionsRoot, CODEX_BIN: sleepStub },
      );
