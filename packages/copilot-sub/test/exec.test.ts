...
const warningStub = join(import.meta.dirname, "fixtures", "stub-copilot-warning.mjs");
...
describe("exec", () => {
  it("writes a correct envelope", async () => {
    await withTempSessions(async (sessionsRoot) => {
      ...
      expect(envelope.backend).toBe("copilot");
      expect(envelope.status).toBe("ok");
      expect(envelope.exit_code).toBe(0);
      expect(envelope.session_id).toBe("copilot-session-abc123");
      expect(envelope.model).toBe("gpt-5.5");
      expect(envelope.result).toBe("Done! Here is the result.");
      expect(envelope.usage).toMatchObject({ premiumRequests: 7.5 });
  ...
  it("rejects COPILOT_BIN pointing at a directory", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const badBin = join(sessionsRoot, "bin-dir");
      await mkdir(badBin);
      const { stdout, code } = await runCli(
        ["--dir", sessionsRoot, "exec", "test"],
        { COPILOT_SUB_HOME: sessionsRoot, COPILOT_BIN: badBin },
      );
      expect(code).toBe(1);
