...
const failStub = join(import.meta.dirname, "fixtures", "stub-copilot-fail.mjs");
const mustNotRunStub = join(
  import.meta.dirname,
  "fixtures",
  "stub-copilot-must-not-run.mjs",
);
...
describe("prompt input", () => {
  it("exec reads prompt from piped stdin when positional is '-'", async () => {
    await withTempSessions(async (sessionsRoot) => {
      ...
      expect(meta.argv[meta.argv.indexOf("-p") + 1]).toBe("piped stdin prompt");
    });
  });
  ...
  it("start with large stdin prompt survives to meta.json and backend argv", async () => {
    await withTempSessions(async (sessionsRoot) => {
      ...
      expect(meta.argv[meta.argv.indexOf("-p") + 1]).toBe(largePrompt);
    });
  });

  it("exec reads prompt from --prompt-file", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const promptDir = await mkdtemp(join(tmpdir(), "copilot-sub-prompt-"));
      ...
      expect(meta.argv[meta.argv.indexOf("-p") + 1]).toBe("file prompt content");
    });
  });

  it("exec accepts a flag-like prompt from --prompt-file", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const promptDir = await mkdtemp(join(tmpdir(), "copilot-sub-prompt-"));
      const promptPath = join(promptDir, "task.txt");
      const argvPath = join(promptDir, "argv.json");
      await writeFile(promptPath, "--model", "utf8");

      const { stdout, code } = await runCli(
        ["--dir", sessionsRoot, "exec", "--prompt-file", promptPath],
        { COPILOT_SUB_HOME: sessionsRoot, STUB_ARGV_FILE: argvPath },
      );
      ...
      expect(meta.prompt).toBe("--model");
      expect(meta.model).toBeNull();
      expect(meta.argv).toContain("-p=--model");
      expect(meta.argv).not.toContain("--model");

      const stubArgv = JSON.parse(await readFile(argvPath, "utf8")) as string[];
      expect(stubArgv).toContain("-p=--model");
      expect(stubArgv).not.toContain("--model");
    });
  });
  ...
  it("allows passthrough flags after '-' positional", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const { code } = await runCli(
        ["--dir", sessionsRoot, "exec", "-", "--mode", "plan"],
        { COPILOT_SUB_HOME: sessionsRoot },
        undefined,
        "stdin with flags",
      );
