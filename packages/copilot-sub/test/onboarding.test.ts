...
describe("onboarding", () => {
  it("bare invocation exits 0 and prints section headers", async () => {
    ...
    expect(stdout).toContain("copilot-subagent");
    expect(stdout).toContain("--allow-all-tools");
    expect(stdout).toContain("--no-auto-update");
    expect(stdout).toContain("--no-ask-user");
    expect(stdout).toContain("--mode");
    expect(stdout).toContain("json");
    expect(stdout).toContain("premiumRequests");
...
describe("docs", () => {
  ...
  it("prints non-empty schema topic", async () => {
    ...
    expect(stdout).toContain("stream-reported model");
  ...
  it("prints non-empty events topic", async () => {
    ...
    expect(code).toBe(0);
    expect(stdout).toContain("assistant.message");
    expect(stdout).toContain("tool.execution_start");
    expect(stdout).toContain("result {sessionId, exitCode, usage}");
  ...
  it("prints non-empty examples topic", async () => {
    ...
    expect(stdout).toContain("jq -r 'select(.t==\"message\") | .text'");
    expect(stdout.length).toBeGreaterThan(200);
  });
});
