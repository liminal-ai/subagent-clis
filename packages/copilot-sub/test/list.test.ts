...
describe("list", () => {
  it("lists recent runs as JSONL with session_id", async () => {
    await withTempSessions(async (sessionsRoot) => {
      ...
      expect(runs[0]!.session_id).toBe("copilot-session-abc123");
  ...
  it("scopes list and latest resolution to cwd by default", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const cwdA = await mkdtemp(join(tmpdir(), "copilot-sub-cwd-a-"));
      const cwdB = await mkdtemp(join(tmpdir(), "copilot-sub-cwd-b-"));
      const cwdC = await mkdtemp(join(tmpdir(), "copilot-sub-cwd-c-"));
      try {
        await runCli(
  ...
  it("stops a running detached run and writes error envelope", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const start = await runCli(
        ...
        { COPILOT_SUB_HOME: sessionsRoot, COPILOT_BIN: sleepStub },
      );
