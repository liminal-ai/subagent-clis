...
const slowStub = join(__dirname, "fixtures", "stub-copilot-slow.mjs");

describe("passthrough", () => {
  it("passes bare --mode plan on exec", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const { code } = await runCli(
        ["--dir", sessionsRoot, "exec", "plan test", "--mode", "plan"],
      ...
      expect(meta.argv).toContain("--allow-all-tools");
      expect(meta.argv).not.toContain("--allow-all-paths");
      expect(meta.argv).not.toContain("--allow-all-urls");
    });
  });

  it("appends --allow-all-tools by default", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const { code } = await runCli(
  ...
  it("keeps --allow-all-tools when a flag value looks like a mode flag", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const { code } = await runCli(
        [
          "--dir",
      ...
      expect(meta.argv).toContain("--allow-all-tools");
      expect(meta.argv).toContain("--agent");
      expect(meta.argv).toContain("--mode");
    });
  });
});
  ...
  it("appends stream.jsonl incrementally before process exit", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const start = await runCli(
        ...
        { COPILOT_SUB_HOME: sessionsRoot, COPILOT_BIN: slowStub },
      );
