...
const sleepStub = join(import.meta.dirname, "fixtures", "stub-copilot-sleep.mjs");
const heldStub = join(import.meta.dirname, "fixtures", "stub-copilot-held.mjs");
...
describe("start/status/result lifecycle", () => {
  ...
  it("start with bare passthrough --available-tools writes meta.json and envelope", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const start = await runCli(
        ["--dir", sessionsRoot, "start", "tools test", "--available-tools", "read_file,grep"],
        { COPILOT_SUB_HOME: sessionsRoot },
      );
      expect(start.code).toBe(0);
      const started = JSON.parse(start.stdout.trim());
      ...
      expect(meta.argv).toContain("--available-tools");
  ...
  it("passes --resume through to Copilot argv", async () => {
    await withTempSessions(async (sessionsRoot) => {
      ...
      const sessionIdx = meta.argv.indexOf("copilot-session-xyz");
      const promptIdx = meta.argv.indexOf("continue please");

      expect(meta.argv[0]).toBe("copilot");
...
describe("last/messages/tools", () => {
  it("prints last and messages", async () => {
    await withTempSessions(async (sessionsRoot) => {
      ...
      expect(messages.stdout.trim()).toBe("Done! Here is the result.");
    });
  });

  it("prints Copilot tool_call and tool_result events as JSONL", async () => {
    await withTempSessions(async (sessionsRoot) => {
      const exec = await runCli(
