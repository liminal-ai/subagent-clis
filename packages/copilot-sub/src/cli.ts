...
import { copilotExists, runSession, spawnDetachedRunner } from "./runner.js";
...
async function requireCopilot(): Promise<void> {
  if (!(await copilotExists())) {
    printErrorJson("Copilot not found", {
      install: "Install Copilot CLI and ensure `copilot` is on PATH",
    });
  }
}
...
program
  ...
  .description("Thin CLI wrapper around Copilot with a documented file substrate")
  .option("--dir <path>", "sessions root override (also COPILOT_SUB_HOME env)")
